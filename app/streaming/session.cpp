...
namespace {
struct DecoderAvailabilityCacheKey
{
    ...
    bool operator==(const DecoderAvailabilityCacheKey& other) const
    {
        return decoderSelection == other.decoderSelection &&
...
struct DecoderPropertiesCacheValue
{
    int capabilities;
    int colorSpace;
    int colorRange;
...
{
    seed = mixHash(seed, key.decoderSelection);
    seed = mixHash(seed, key.videoFormat);
    seed = mixHash(seed, key.width);
    seed = mixHash(seed, key.height);
    seed = mixHash(seed, key.frameRate);
    return seed;
}

QHash<DecoderAvailabilityCacheKey, int> s_DecoderAvailabilityCache;
...
void Session::clConnectionTerminated(int errorCode)
{
    ...
    if (s_ActiveSession->m_SuppressTerminationErrors) {
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                    "Suppressing termination dialog due to pending auto reconnect: %d",
                    errorCode);
...
struct ClipboardTransferState
{
    ...
    std::atomic_uint64_t localGeneration {0};
...
namespace {
    ...
    constexpr int ClipboardFilePublishTimeoutMs = 60 * 1000;

    qint64 clipboardTraceElapsed(qint64 traceStartedMs)
    {
        return traceStartedMs > 0 ?
                    std::max<qint64>(
                        0,
                                     ...
                                     ClipboardHttpContext& context)
    {
        ...
        context.address = computer->activeAddress;
        context.httpsPort = computer->activeHttpsPort;
        context.serverCertificate = computer->serverCert;
        context.useTrueUid = !computer->isNvidiaServerSoftware;
        return !context.address.isNull() &&
                context.httpsPort != 0 &&
                !context.serverCertificate.isNull();
    }

    bool pushClipboardEvent(int code, void* data)
    {
        SDL_Event event = {};
        event.type = SDL_USEREVENT;
    ...
    {
        ClipboardReadyEvent* readyEvent = new ClipboardReadyEvent {
            transferState,
        };
        if (!pushClipboardEvent(SDL_CODE_CLIPBOARD_READY, readyEvent)) {
            delete readyEvent;
        }
    }

    class ClipboardBlobUploadTask : public QRunnable
    {
    public:
        ClipboardBlobUploadTask(ClipboardHttpContext httpContext,
                                QByteArray content,
                                quint8 targetMimeType,
                                quint64 originId,
                                QByteArray contentKey,
        ...
        void run() override
        {
            ...
            try {
                ...
                const auto result = http.uploadClipboardBlob(
                            m_TargetMimeType == LI_CLIPBOARD_MIME_PNG ?
                ...
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                            "Clipboard blob upload failed: %s",
                            error.what());
            }
        }
    ...
    private:
        bool isCurrent() const
        {
            return m_TransferState->active.load() &&
                    m_TransferState->localGeneration.load() == m_Generation;
        }
    ...
    class ClipboardBlobDownloadTask : public QRunnable
    {
    public:
        ClipboardBlobDownloadTask(ClipboardHttpContext httpContext,
                                  QString id,
                                  quint8 targetMimeType,
        ...
        void run() override
        {
            ...
            try {
                NvHTTP http(m_HttpContext.address,
                            ...
                            m_HttpContext.serverCertificate,
                            m_HttpContext.useTrueUid);
                QByteArray content = http.downloadClipboardBlob(
                            m_Id,
                            m_TargetMimeType == LI_CLIPBOARD_MIME_PNG ?
                                QByteArrayLiteral("image/png") :
                                QByteArrayLiteral("text/plain"),
                ...
                const bool valid =
                        m_TargetMimeType == LI_CLIPBOARD_MIME_TEXT_UTF8 ?
                            LiIsValidUtf8ClipboardText(
                                reinterpret_cast<const uint8_t*>(content.constData()),
                                static_cast<size_t>(content.size())) :
                            LiIsValidClipboardPngHeader(
                                reinterpret_cast<const uint8_t*>(content.constData()),
                                static_cast<size_t>(content.size()));
                if (!valid) {
                    throw std::runtime_error("Invalid clipboard blob content");
                }
                ...
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                            "Clipboard blob download failed: %s",
                            error.what());
            }
        }
        ...
        void run() override
        {
            ...
            try {
                ...
                public:
                    ...
                    {
                        try {
                            m_Http.releaseClipboardFileSource(
                                        m_Id,
                                        m_OriginId);
                        }
                        catch (...) {
                ...
                private:
                    ...
                    QString m_Id;
                    quint64 m_OriginId;
                } sourceLease(http, result.id, m_OriginId);
                SDL_LogDebug(
                            SDL_LOG_CATEGORY_APPLICATION,
                            "Clipboard file trace generation=%llu transfer=%s phase=registered stage_ms=%lld total_ms=%lld",
...
void Session::clClipboardContent(PSS_CLIPBOARD_CONTENT content)
{
    ...
    const auto transferState = session->m_ClipboardTransferState;
    const auto hostCapabilities = session->m_ClipboardHostCapabilities.load();
    if (!transferState->active.load() ||
            (hostCapabilities & LI_CLIPBOARD_CAP_CAN_SEND) == 0) {
        return;
    }
    ...
    if (content->mimeType == LI_CLIPBOARD_MIME_BLOB_REFERENCE) {
        ...
        const quint64 requestOriginId = LiGetClipboardOriginId();
        if (requestOriginId == 0 ||
                !captureClipboardHttpContext(session->m_Computer, httpContext)) {
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                        "Clipboard blob download is unavailable");
            return;
        }
    ...
    clipboardEvent->data = QByteArray(reinterpret_cast<const char*>(content->data),
                                      static_cast<int>(content->length));
    clipboardEvent->generation = generation;
    clipboardEvent->transferState = transferState;

    if (!pushClipboardEvent(SDL_CODE_CLIPBOARD_CONTENT, clipboardEvent)) {
        delete clipboardEvent;
    }
}
...
void Session::clClipboardStatus(PSS_CLIPBOARD_STATUS status)
{
    Session* session = s_ActiveSession;
    if (session == nullptr ||
            status == nullptr ||
    ...
    if (!pushClipboardEvent(SDL_CODE_CLIPBOARD_STATUS, statusEvent)) {
        delete statusEvent;
    }
}
...
void Session::applyRemoteClipboardContent(const ClipboardEvent* clipboardEvent)
{
    if (!m_StreamConfig.enableClipboardSync ||
            ...
            clipboardEvent->transferState != m_ClipboardTransferState ||
            !m_ClipboardTransferState->active.load() ||
            clipboardEvent->generation !=
                m_ClipboardTransferState->remoteGeneration.load()) {
        return;
    }
    ...
    if (clipboardEvent->mimeType == LI_CLIPBOARD_MIME_TEXT_UTF8) {
        ...
        mimeData->setData("image/png", clipboardEvent->data);
        ...
        const ClipboardBlobUploadEvent* uploadEvent)
{
    ...
    const QByteArray id = uploadEvent->id.toLatin1();
    quint64 itemId = 0;
    if (LiSendClipboardBlobReferenceEx(
                uploadEvent->targetMimeType,
                id.constData(),
                uploadEvent->size,
                reinterpret_cast<const uint8_t*>(
                    uploadEvent->sha256.constData()),
                &itemId) != 0) {
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
...
void Session::completeClipboardStatus(
        const ClipboardStatusEvent* statusEvent)
{
#ifdef Q_OS_WIN32
    ...
    if (m_FileClipboardGeneration !=
            m_ClipboardTransferState->localGeneration.load() ||
            m_FileClipboardSequence == 0 ||
            m_FileClipboardSequence != GetClipboardSequenceNumber()) {
        resetFileClipboardPublication();
        return;
    }

    m_FileClipboardPublishState = FileClipboardPublishState::Ready;
    rememberClipboardContent(m_FileClipboardContentKey);
    SDL_LogDebug(
                SDL_LOG_CATEGORY_APPLICATION,
                "Clipboard file trace generation=%llu phase=host_ack item=%llu total_ms=%lld",
                static_cast<unsigned long long>(
    ...
    if (m_DeferredFilePasteRequested) {
        sendDeferredFilePaste();
    }
#else
    Q_UNUSED(statusEvent);
#endif
}
...
void Session::sendCurrentClipboardContent(bool initialSync)
{
    ...
    if ((hostCapabilities &
        ...
        if (allLocalFiles) {
#ifdef Q_OS_WIN32
            const quint32 clipboardSequence = GetClipboardSequenceNumber();
            if (clipboardSequence == m_FileClipboardSequence &&
                    m_FileClipboardPublishState !=
            ...
            m_FileClipboardSequence = clipboardSequence;
            m_FileClipboardGeneration = generation;
            if (m_FileClipboardTraceStartedMs == 0) {
                m_FileClipboardTraceStartedMs =
                        QDateTime::currentMSecsSinceEpoch();
            }
    ...
    if ((hostCapabilities & LI_CLIPBOARD_CAP_CAN_RECEIVE) != 0 &&
        ...
        if (!image.isNull() &&
                static_cast<quint64>(image.width()) * image.height() <= LI_CLIPBOARD_MAX_IMAGE_PIXELS) {
            QByteArray png = mimeData->data("image/png");
            if (png.isEmpty() ||
                    !LiIsValidClipboardPngHeader(
                ...
                if (!buffer.open(QIODevice::WriteOnly) ||
                    ...
                    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                                "Failed to encode clipboard image as PNG");
                    return;
                }
            }
            const QByteArray contentKey = clipboardContentKey(
            ...
            if (png.size() <= ClipboardMaxBlobBytes &&
                    (localCapabilities & LI_CLIPBOARD_CAP_BLOB) != 0 &&
                    (hostCapabilities & LI_CLIPBOARD_CAP_BLOB) != 0) {
                const quint64 originId = LiGetClipboardOriginId();
                ClipboardHttpContext httpContext;
                if (originId != 0 &&
                    ...
                    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                                "Clipboard blob upload is unavailable");
                }
                return;
            }

            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                        "Clipboard PNG exceeds the negotiated size limit");
        }
        return;
    }
...
bool Session::handleFileClipboardPasteKey(
        const SDL_KeyboardEvent* event)
{
#ifdef Q_OS_WIN32
    if (event == nullptr || event->keysym.scancode != SDL_SCANCODE_V) {
        return false;
    }
    if (m_SuppressFilePasteKey) {
        if (event->state == SDL_RELEASED) {
            m_SuppressFilePasteKey = false;
        }
        return true;
    }
    ...
    if (clipboardSequence == m_FileClipboardSequence &&
            m_FileClipboardPublishState ==
                FileClipboardPublishState::Ready) {
        return false;
    }
    if (clipboardSequence != m_FileClipboardSequence) {
        resetFileClipboardPublication();
    ...
    m_DeferredFilePasteRequested = true;
    m_SuppressFilePasteKey = true;
    const quint64 pasteToken = ++m_DeferredFilePasteToken;
    QTimer::singleShot(
                ClipboardFilePublishTimeoutMs,
                this,
                [this, pasteToken]() {
                    if (pasteToken == m_DeferredFilePasteToken &&
                            m_DeferredFilePasteRequested) {
                        ++m_ClipboardTransferState->localGeneration;
                        resetFileClipboardPublication();
                        SDL_LogWarn(
                                    SDL_LOG_CATEGORY_APPLICATION,
                                    "Timed out waiting for host clipboard file acknowledgement");
                    }
                });

    if (m_FileClipboardPublishState ==
            FileClipboardPublishState::Idle) {
        m_FileClipboardTraceStartedMs =
                QDateTime::currentMSecsSinceEpoch();
...
void Session::sendDeferredFilePaste()
{
#ifdef Q_OS_WIN32
    if (!m_DeferredFilePasteRequested ||
            !m_ClipboardTransferState->active.load() ||
    ...
    m_DeferredFilePasteRequested = false;
    ++m_DeferredFilePasteToken;
    SDL_LogDebug(
                SDL_LOG_CATEGORY_APPLICATION,
                "Clipboard file trace generation=%llu phase=deferred_paste_sent total_ms=%lld",
                static_cast<unsigned long long>(
                    m_FileClipboardGeneration),
                static_cast<long long>(
                    clipboardTraceElapsed(
                        m_FileClipboardTraceStartedMs)));
    constexpr short LeftControlKey = 0xA2;
    constexpr short VKey = 0x56;
    LiSendKeyboardEvent2(
                static_cast<short>(0x8000 | LeftControlKey),
                KEY_ACTION_DOWN,
                MODIFIER_CTRL,
                0);
    LiSendKeyboardEvent2(
                static_cast<short>(0x8000 | VKey),
                KEY_ACTION_DOWN,
                MODIFIER_CTRL,
                0);
    LiSendKeyboardEvent2(
...
void Session::resetFileClipboardPublication()
{
    if (m_FileClipboardPublishState !=
            FileClipboardPublishState::Idle &&
            m_FileClipboardGeneration != 0 &&
            m_ClipboardTransferState->localGeneration.load() ==
                m_FileClipboardGeneration) {
        ++m_ClipboardTransferState->localGeneration;
    }
    m_FileClipboardPublishState = FileClipboardPublishState::Idle;
    m_FileClipboardSequence = 0;
    m_FileClipboardItemId = 0;
    m_FileClipboardGeneration = 0;
    m_FileClipboardTraceStartedMs = 0;
    m_FileClipboardContentKey.clear();
    m_DeferredFilePasteRequested = false;
    ++m_DeferredFilePasteToken;
}
...
void Session::deactivateClipboardSync()
{
    ...
    ++m_ClipboardTransferState->localGeneration;
    ++m_ClipboardTransferState->remoteGeneration;
    resetFileClipboardPublication();
    m_SuppressFilePasteKey = false;
}
                            ...
                            int frameRate, bool enableVsync, bool enableFramePacing, bool testOnly, IVideoDecoder*& chosenDecoder)
{
    ...
    params.skipTestFrame = false;
    params.vds = vds;

    if (!testOnly && s_ActiveSession != nullptr &&
            s_ActiveSession->m_PrevalidatedDecoderSelection == static_cast<int>(vds) &&
            s_ActiveSession->m_PrevalidatedVideoFormat == videoFormat &&
            s_ActiveSession->m_PrevalidatedVideoWidth == width &&
            s_ActiveSession->m_PrevalidatedVideoHeight == height &&
            s_ActiveSession->m_PrevalidatedVideoFrameRate == frameRate) {
        params.skipTestFrame = true;
    }
    ...
    else if (!testOnly && params.skipTestFrame) {
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                    "Fast decoder initialization failed; retrying with test frame");
        delete chosenDecoder;
        chosenDecoder = new FFmpegVideoDecoder(testOnly);
        params.skipTestFrame = false;
        if (chosenDecoder->initialize(&params)) {
...
Session::DecoderAvailability
Session::getCachedDecoderAvailability(SDL_Window* window,
                                      StreamingPreferences::VideoDecoderSelection vds,
                                      int videoFormat, int width, int height, int frameRate)
{
    DecoderAvailabilityCacheKey key = {
        static_cast<int>(vds),
        videoFormat,
        width,
        height,
    ...
    {
        QMutexLocker locker(&s_DecoderCacheLock);
        auto it = s_DecoderAvailabilityCache.constFind(key);
        if (it != s_DecoderAvailabilityCache.cend()) {
            return static_cast<DecoderAvailability>(it.value());
        }
    }
    ...
    bool alwaysFullScreen;
    bool hardwareAccelerated;
    if (!getCachedDecoderProperties(window, vds, videoFormat, width, height, frameRate,
                                    capabilities, colorSpace, colorRange, alwaysFullScreen,
                                    hardwareAccelerated)) {
        QMutexLocker locker(&s_DecoderCacheLock);
        s_DecoderAvailabilityCache.insert(key, static_cast<int>(DecoderAvailability::None));
        return DecoderAvailability::None;
    }

    return hardwareAccelerated ? DecoderAvailability::Hardware : DecoderAvailability::Software;
}

bool
Session::getCachedDecoderProperties(SDL_Window* window,
                                    StreamingPreferences::VideoDecoderSelection vds,
                                    int videoFormat,
                                    int width,
                                    int height,
                                    ...
                                    bool& hardwareAccelerated)
{
    ...
    {
        QMutexLocker locker(&s_DecoderCacheLock);
        auto it = s_DecoderPropertiesCache.constFind(key);
        if (it != s_DecoderPropertiesCache.cend()) {
            capabilities = it.value().capabilities;
            colorSpace = it.value().colorSpace;
            colorRange = it.value().colorRange;
            alwaysFullScreen = it.value().alwaysFullScreen;
            hardwareAccelerated = it.value().hardwareAccelerated;
            return true;
        }
    }
    ...
    DecoderPropertiesCacheValue value = {
        decoder->getDecoderCapabilities(),
        decoder->getDecoderColorspace(),
        decoder->getDecoderColorRange(),
    ...
    {
        QMutexLocker locker(&s_DecoderCacheLock);
        s_DecoderPropertiesCache.insert(key, value);
        s_DecoderAvailabilityCache.insert(key, value.hardwareAccelerated ? static_cast<int>(DecoderAvailability::Hardware)
                                                                         : static_cast<int>(DecoderAvailability::Software));
    }

    capabilities = value.capabilities;
    colorSpace = value.colorSpace;
    colorRange = value.colorRange;
    alwaysFullScreen = value.alwaysFullScreen;
    hardwareAccelerated = value.hardwareAccelerated;
    return true;
}

void Session::prewarmDecoderCache(SDL_Window* window,
                                  StreamingPreferences::VideoDecoderSelection vds,
                                  ...
                                  int frameRate)
{
    ...
    case StreamingPreferences::VCC_FORCE_H264:
        videoFormat = enableYUV444 ? VIDEO_FORMAT_H264_HIGH8_444 : VIDEO_FORMAT_H264;
        break;
    case StreamingPreferences::VCC_FORCE_AV1:
        videoFormat = enableYUV444 ? (enableHdr ? VIDEO_FORMAT_AV1_HIGH10_444 : VIDEO_FORMAT_AV1_HIGH8_444) :
                                     (enableHdr ? VIDEO_FORMAT_AV1_MAIN10 : VIDEO_FORMAT_AV1_MAIN8);
        break;
    case StreamingPreferences::VCC_FORCE_HEVC:
    case StreamingPreferences::VCC_FORCE_HEVC_HDR_DEPRECATED:
    case StreamingPreferences::VCC_AUTO:
    default:
        videoFormat = enableYUV444 ? (enableHdr ? VIDEO_FORMAT_H265_REXT10_444 : VIDEO_FORMAT_H265_REXT8_444) :
                                     (enableHdr ? VIDEO_FORMAT_H265_MAIN10 : VIDEO_FORMAT_H265);
        break;
    }
    ...
    bool alwaysFullScreen;
    bool hardwareAccelerated;
    getCachedDecoderProperties(window, vds, videoFormat, width, height, frameRate,
                               capabilities, colorSpace, colorRange, alwaysFullScreen,
                               hardwareAccelerated);
}
...
bool Session::populateDecoderProperties(SDL_Window* window)
{
    ...
    if (!getCachedDecoderProperties(window,
    ...
    m_PrevalidatedDecoderSelection = static_cast<int>(m_Preferences->videoDecoderSelection);
    m_PrevalidatedVideoFormat = m_SupportedVideoFormats.first();
    m_PrevalidatedVideoWidth = m_StreamConfig.width;
    m_PrevalidatedVideoHeight = m_StreamConfig.height;
    m_PrevalidatedVideoFrameRate = m_StreamConfig.fps;
    ...
    if (Utils::getEnvironmentVariableOverride("COLOR_SPACE_OVERRIDE", &m_StreamConfig.colorSpace)) {
        ...
        m_StreamConfig.colorSpace = colorSpace;
    }
    ...
    if (Utils::getEnvironmentVariableOverride("COLOR_RANGE_OVERRIDE", &m_StreamConfig.colorRange)) {
        ...
        m_StreamConfig.colorRange = colorRange;
    }
...
Session::Session(NvComputer* computer, NvApp& app, StreamingPreferences *preferences)
    : m_Preferences(preferences ? preferences : StreamingPreferences::get()),
      ...
      m_PrevalidatedDecoderSelection(-1),
      m_PrevalidatedVideoFormat(0),
      m_PrevalidatedVideoWidth(0),
      m_PrevalidatedVideoHeight(0),
      m_PrevalidatedVideoFrameRate(0),
...
bool Session::shouldAutoReconnect() const
{
    return m_AutoReconnectPending;
}
...
bool Session::initialize(QQuickWindow* qtWindow)
{
    ...
    case StreamingPreferences::VCC_AUTO:
    {
        ...
        auto hevcDA = getCachedDecoderAvailability(testWindow,
                                                   m_Preferences->videoDecoderSelection,
        ...
        if (hevcDA == DecoderAvailability::None && m_Preferences->enableHdr) {
            ...
            auto av1DA = getCachedDecoderAvailability(testWindow,
                                                      m_Preferences->videoDecoderSelection,
            ...
            if (av1DA == DecoderAvailability::None) {
                ...
                hevcDA = getCachedDecoderAvailability(testWindow,
                                                      m_Preferences->videoDecoderSelection,
        ...
        if (hevcDA == DecoderAvailability::Hardware
            ...
            m_SupportedVideoFormats.deprioritizeByMask(VIDEO_FORMAT_MASK_AV1);
                   ...
                   getCachedDecoderAvailability(testWindow,
                                                m_Preferences->videoDecoderSelection,
    ...
    if (m_Preferences->enableHdr) {
        ...
        // Check that the server GPU supports HDR
            ...
            if (m_SupportedVideoFormats.maskByServerCodecModes(m_Computer->serverCodecModeSupport & SCM_AV1_MAIN10)) {
                auto da = getCachedDecoderAvailability(testWindow,
                                                       m_Preferences->videoDecoderSelection,
...
bool Session::startConnectionAsync()
{
    ...
    try {
...

        {
            ...
            const bool preferManualAddress =
                    !m_Computer->manualAddress.isNull() &&
                    NvComputer::isWrongSubnetSiteLocalAddress(m_Computer->localAddress);

            if (preferManualAddress && m_Computer->activeAddress != m_Computer->manualAddress) {
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                            "Preferring manual address over local address for session start: %s -> %s",
                            m_Computer->activeAddress.toString().toUtf8().constData(),
                            m_Computer->manualAddress.toString().toUtf8().constData());
                m_Computer->activeAddress = m_Computer->manualAddress;
            }

            http.setAddress(m_Computer->activeAddress);
            if (m_Computer->activeHttpsPort != 0) {
                http.setHttpsPort(m_Computer->activeHttpsPort);
            }
...
void Session::triggerAutoReconnect(const char* reason)
{
    if (m_AutoReconnectPending || m_ShouldExit) {
        return;
    }

    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                "Triggering auto reconnect due to: %s",
                reason ? reason : "unknown");

    m_AutoReconnectPending = true;
    m_SuppressTerminationErrors = true;
...
void Session::exec()
{
    ...
    SDL_Event event;
    Uint64 lastLoopTick = SDL_GetTicks64();
    for (;;) {
        Uint64 currentLoopTick = SDL_GetTicks64();
        ...
        if (m_Preferences->autoReconnectAfterResume && loopGap >= AUTO_RECONNECT_SUSPEND_GAP_MS) {
            ...
            triggerAutoReconnect("main loop gap");
        }
        ...
        case SDL_USEREVENT:
            ...
            {
                ClipboardReadyEvent* readyEvent =
                        static_cast<ClipboardReadyEvent*>(event.user.data1);
                if (readyEvent->transferState == m_ClipboardTransferState &&
                        m_ClipboardTransferState->active.load() &&
                        m_ClipboardSyncReady.load()) {
                    sendCurrentClipboardContent(true);
                }
                delete readyEvent;
                break;
            }
            case SDL_CODE_CLIPBOARD_BLOB_UPLOADED:
            {
                ClipboardBlobUploadEvent* uploadEvent =
                        static_cast<ClipboardBlobUploadEvent*>(event.user.data1);
                completeClipboardBlobUpload(uploadEvent);
                delete uploadEvent;
                break;
            }
            ...
            {
                ClipboardStatusEvent* statusEvent =
                        static_cast<ClipboardStatusEvent*>(event.user.data1);
                completeClipboardStatus(statusEvent);
                delete statusEvent;
                break;
            }
            default:
        ...
        case SDL_KEYDOWN:
            presence.runCallbacks();
            if (!handleFileClipboardPasteKey(&event.key)) {
